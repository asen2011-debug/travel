...
/** 去程 + 环线（方案 A/B/C 共用，方案 D 替换 D6-D7）。实际 9/20 已出发 */
const BASE_DAYS: DayPlan[] = [
  ...
  },
  {
    ...
    note: '莫高窟 A 类票大概率已售罄；9/26 起在"莫高窟参观预约网"抢 B 类应急票（6000 张/天），或当天早 8 点现场排队',
    phase: 'loop',
...
const RETURN_A: DayPlan[] = [
  {
    ...
    note: '全国高速出城高峰从下午开始，上午出发；今天高速尚未免费（小额通行费）',
    phase: 'return',
  ...
  },
  {
    ...
    note: '扎尕那正值国庆最高峰，住宿务必提前订，当天早进景区；高速起免费',
    phase: 'return',
  ...
  },
  {
    ...
    mileageKm: 550,
    driveHours: 6.5,
    highlights: ['傍晚到家', '10/5-10/7 在家休息 3 天'],
    note: '7:00 前出发，中午前过洛阳，避开返程车流',
    phase: 'return',
...
const RETURN_B: DayPlan[] = [
  RETURN_A[0],
  {
    ...
    note: '国庆首日，若尔盖方向车流增大，早出发；10 月初草原已枯黄',
    phase: 'return',
...
/** 方案 C 回程：原路高速返回（9/30 先短途到兰州休整，10/1 起高速免费） */
const RETURN_C: DayPlan[] = [
  {
    ...
    note: '轻松短途日；今天高速仍收费（约 100 元），明天起免费',
    phase: 'return',
  ...
  },
  {
    ...
    mileageKm: 630,
    driveHours: 7.5,
    highlights: ['全高速返程（国庆免费）'],
    note: '假期首日上午出发，避开午后高峰',
    phase: 'return',
...
const LOOP_VARIANT_D: DayPlan[] = [
  {
    ...
    note: '中秋节在格尔木；海拔从 2780m 猛升至 4768m，高反风险大，带氧气瓶，量力而行',
    phase: 'loop',
...
export const ROUTE_OPTIONS: RouteOption[] = [
  {
    ...
    recommended: true,
    description:
      '青藏高原东北缘：草原、峡谷、藏寨、寺庙，与环线的盐湖戈壁沙漠丹霞完全不同，与去程零重复。海拔 2000-3500m，不深入藏区。10/4 到家，还能休息 3 天。',
    pros: ['风景类型与环线互补', '与去程零重复', '10/4 到家，休息 3 天', '5 天从容，含 2 天纯赶路'],
    cons: ['扎尕那 10/1 正值国庆最高峰，住宿需早订', '迭部—陇南段山路弯多'],
  ...
  },
  {
    ...
    distanceKm: 1400,
    description:
      '西宁 → 兰州 → 西安 → 家，全程高速。9/30 先短途到兰州休整，10/1 起高速免费，10/2 傍晚到家，可在家休息 5 天。完全重复去程，作为恶劣天气、身体不适或时间失控时的兜底方案。',
    pros: ['10/2 到家，在家休息 5 天', '全高速，每天驾驶不超过 7.5 小时', '10/1 起高速免费'],
    cons: ['完全重复去程风景', '无新增景点'],
...
export const KEY_TIPS = [
  '莫高窟 9/27 参观：A 类票大概率已售罄，9/26 起在"莫高窟参观预约网"抢 B 类应急票（或当天早 8 点现场排队）',
  '国庆高速免费：10/1 0:00 - 10/7 24:00，7 座及以下，以出高速时间为准；中秋不免费',
  '10/4 返程 7:00 前出发，中午前过洛阳；到家后 10/5-10/7 休息 3 天',
]
